...
public class ParserGenerator {
        ...
        if (rule.andRules.isEmpty()) {
            ...
            for (AndRule andRule : rule.allAndRules()) {
                ...
                if (andRule.repeatRules.isEmpty()) {
                    ...
                    addRepeatField(newClassName, cb,
                            newClassName.decapName(), RepeatType.Once, REQUIRED);
    ...
    private void addToken(
        ...
        if (classNameMap.containsKey(subRule.token)) {
            ...
            addRepeatField(ClassName.of(classType), cb,
                    ParseStringUtil.decap(classType), repeatType, isOptional);
        } else {
            ...
            if (clsName.equals("boolean")) {
                ...
                addRepeatField(ClassName.of("boolean"), cb, fieldName, repeatType, REQUIRED);
            } else {
                addRepeatField(ClassName.of(clsName), cb,
                        convertedValue.getFieldName(), repeatType, isOptional);
            }
        }
    }
    ...
    private void addRepeatField(
        ...
        if (repeatType == RepeatType.Once || repeatType == RepeatType.OnceOrMore) {
            var field = new ClassField(className, fieldName, isOptional, false);
            cb.addField(field);
        }
        ...
        if (repeatType == RepeatType.OnceOrMore || repeatType == RepeatType.NoneOrMore) {
            ...
            var field = new ClassField(newClassName, newFieldName, false, true);
            cb.addField(field);
        }
    }
    ...
    private void addOrRuleAsComponent(
        ...
        if (rule.andRules.isEmpty() && rule.andRule.repeatRules.isEmpty() &&
            ...
            // The reason to do this first is that if adding the rule fails,
            // this class can still show that this point was reached
            addRepeatField(className, cb, className.decapName(), repeatType, isOptional);
