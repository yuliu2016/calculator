...
public class ParserGenerator {
        ...
        if (rule.andRules.isEmpty()) {
            ...
            for (AndRule andRule : rule.allAndRules()) {
                ...
                if (andRule.repeatRules.isEmpty()) {
                    ...
                    addAndRule(classWithCount, cb, andRule);
                } else {
                    ...
                    cb.addField(classType, ParseStringUtil.decap(classType));

                    addAndRule(classWithCount, component_cb, andRule);
                }
            }
        }
    }

    private void addAndRule(ClassName className, ClassBuilder cb, AndRule rule) {
        ...
        if (rule.repeatRules.isEmpty()) {
            ...
            for (RepeatRule repeatRule : rule.allRepeatRules()) {
                ...
                addRepeatedSubRule(classWithCount, cb, repeatRule);
            }
        }
    }

    private void addRepeatedSubRule(ClassName className, ClassBuilder cb, RepeatRule repeatRule) {
        ...
        if (sub.groupedOrRule != null) {
            ...
            if (classNameMap.containsKey(sub.token)) {
                ...
                if (clsName.equals("boolean")) {
                    ...
                    addFieldWithRepeat(ClassName.of("boolean"), cb, fieldName, repeatRule);
                } else {
                    addFieldWithRepeat(ClassName.of(clsName), cb, convertedValue.getFieldName(), repeatRule);
                }
            }
        }
    }
    ...
    private void addFieldWithRepeat(
            ClassName className, ClassBuilder cb, String fieldName, RepeatRule repeatRule) {

        if (repeatRule.tokenStar) {
            cb.addImport("java.util.List");
            cb.addFieldByClassName(className.wrap("List"), fieldName + "List");
        } else if (repeatRule.tokenPlus) {
            cb.addFieldByClassName(className, fieldName);
            cb.addImport("java.util.List");
            cb.addFieldByClassName(className.wrap("List"), fieldName + "List");
        } else {
            cb.addFieldByClassName(className, fieldName);
    ...
    private void addOrRuleAsComponent(ClassName className, ClassBuilder cb, OrRule rule, RepeatRule repeatRule) {
        ...
        if (rule.andRules.isEmpty() && rule.andRule.repeatRules.isEmpty() &&
            ...
            addFieldWithRepeat(classWithSuffix, cb,
                    ParseStringUtil.decap(classType), repeatRule);
