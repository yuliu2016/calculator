...
public class ParserGenerator {
    ...
    private void validateSubRule(SubRule rule) {
        if (rule.groupedOrRule != null) {
            ...
            if (!ruleMap.containsKey(rule.token) && converter.checkToken(rule.token) == null) {
    ...
    private void addToken(
        ...
        if (classNameMap.containsKey(subRule.token)) {
            ...
            var convertedValue = converter.checkToken(subRule.token);

            if (convertedValue == null) {
                throw new IllegalStateException("SubRule token " +
                        subRule.token + " is invalid!");
            }
